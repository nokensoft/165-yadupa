...
@section('title', 'Galeri Video - ' . ($situs['nama_situs'] ?? 'YADUPA - Yayasan Anak Dusun Papua'))
@section('seo-title', 'Galeri Video - ' . ($situs['nama_situs'] ?? 'YADUPA - Yayasan Anak Dusun Papua'))
@section('seo-description', 'Dokumentasi video kegiatan, sarana prasarana, dan agenda YADUPA - Yayasan Anak Dusun Papua.')
...
<section class="relative bg-gradient-to-br from-primary-800 via-primary-900 to-secondary-900 h-[150px] flex items-center justify-center overflow-hidden">
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-white">
        <h1 class="text-4xl sm:text-5xl font-extrabold tracking-tight">Galeri Video</h1>
    </div>
</section>
...
<section class="py-12 bg-gray-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        @foreach($videos as $video)
                            <div class="bg-white border border-gray-200 overflow-hidden flex flex-col transition-all duration-200 hover:border-gray-400">
                                {{-- Container Cover Video & Icon Play --}}
                                <a href="{{ route('video.show', $video->slug) }}" class="relative w-full aspect-video bg-gray-100 overflow-hidden group block">
                                    <img 
                                        src="{{ $video->gambar }}" 
                                        alt="{{ $video->title }}" 
                                    ...
                                    <div class="absolute inset-0 bg-black/30 flex items-center justify-center group-hover:bg-black/20 transition-colors">
                                        <div class="w-12 h-12 bg-red-600 text-white rounded-full flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform">
                                ...
                                <div class="p-4 flex-1 flex flex-col justify-between">
                                    <a href="{{ route('video.show', $video->slug) }}" class="hover:text-primary-800 transition-colors">
                                    ...
                                    <div class="mt-3 pt-2 border-t border-gray-100 flex items-center justify-between text-xs text-gray-500">
                                        <span>{{ $video->created_at ? $video->created_at->translatedFormat('d M Y') : '-' }}</span>
                                        <a href="{{ route('video.show', $video->slug) }}" class="font-semibold text-primary-900 hover:underline">
                                            Tonton Video &rarr;
                                        </a>
                                    </div>
                                </div>
            ...
            {{-- Kolom Kanan: Sidebar Kategori Video & Form Pencarian --}}
            <div class="space-y-6">
                {{-- Form Pencarian --}}
                <div class="bg-white border border-gray-200 p-4">
                    ...
                    <form action="{{ route('video.index') }}" method="GET" class="flex gap-2">
                        @if(request('category'))
                ...
                <div class="bg-white border border-gray-200 p-4">
                    ...
                    <div class="flex flex-col gap-2">
                        <a 
                            href="{{ route('video.index', array_filter(['q' => request('q')])) }}"
                        ...
                        @foreach($categories as $cat)
                            <a 
                                href="{{ route('video.index', array_filter(['category' => $cat, 'q' => request('q')])) }}"
